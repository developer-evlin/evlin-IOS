...
struct ChildLockableHubStep: View {
    ...
    // Not FamilyActivitySelection/.familyActivityPicker (the real Screen
    // Time picker) — that requires genuine, granted Family Controls
    // authorization to actually register a selection, which the earlier
    // ChildScreenTimeStep only fakes at the UI-state level (see its "Try
    // Again" comment). On the Simulator especially, the real picker would
    // present but tapping apps in it would silently do nothing, leaving
    // totalSelected stuck at 0 and Continue permanently disabled — exactly
    // what was reported. A mock list, matching every other "backend" call
    // in this file, sidesteps needing real authorization at all.
    @State private var selectedAppIDs: Set<String> = []
    @State private var showPicker = false

    private var totalSelected: Int { selectedAppIDs.count }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showPicker) {
            MockLockableAppPickerSheet(selectedIDs: $selectedAppIDs, onDone: { showPicker = false })
        }
    }
}
...
// Stands in for the real FamilyActivityPicker — a plain checklist of mock
// apps, matching this file's "everything is a local @State flip" design
